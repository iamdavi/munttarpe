<template>
  ...
  <v-row>
    <v-col class="flex-grow-0 flex-shrink-0">
      <v-date-picker :weekdays="[1, 2, 3, 4, 5, 6, 0]" first-day-of-week="1" location="es eu" v-model="selectedDate"
        show-adjacent-months ref="datePicker" elevation="4"></v-date-picker>
      <v-btn color="green-darken-1" prepend-icon="mdi-plus" class="mt-3" variant="outlined" block @click="
        eventType = 'period';
      isDialogOpen = true;
      ">
    ...
    <v-col class="flex-grow-1 flex-shrink-0">
      <v-row>
        <v-col cols="12" class="d-flex justify-space-between align-center">
          ...
          <v-btn variant="outlined" prepend-icon="mdi-plus" color="green-darken-1" @click="
            eventType = 'day';
          isDialogOpen = true;
          ">
        ...
        <v-col lg="3" md="6" sm="12" v-for="(event, n) in databaseStore.eventos" :key="n">
          <EventoCard :evento="event" :equipos="getEquipoDataById(event.equipos)"
  ...
  <v-dialog v-model="deleteEventDialog" max-width="500">
    <v-card title="Eliminar evento?" prepend-icon="mdi-calendar-remove-outline">
      <v-divider></v-divider>
      <EventoCard id="card-delete-preview" :evento="eventToDelete"
        :equipos="getEquipoDataById(eventToDelete?.equipos)" />
      <v-divider></v-divider>
      <v-card-actions class="d-flex justify-space-between">
        <v-btn @click="deleteEventDialog = false" prepend-icon="mdi-close" variant="outlined">
          Cancelar
        </v-btn>
        <v-btn prepend-icon="mdi-trash-can-outline" color="red" variant="outlined" @click="deleteEventAction()"
  ...
  <EventoFormModal :isOpen="isDialogOpen" actionType="crear" :eventType="eventType" :eventDay="formatedDate"
