    ...
    <v-form @submit.prevent="handleModalForm">
      <v-card :prepend-icon="modalType == 'crear' ? 'mdi-handball' : 'mdi-pencil-outline'
          ...
          <v-row>
            <v-col class="pt-0">
              <v-card-text class="p-0">
                <v-row>
                  <v-col cols="12">
                    <v-select v-model="form.equipo" :items="databaseStore.equipos" item-title="nombre" item-value="id"
                      label="Equipo *" :rules="[(v) => !!v || 'Debes seleccionar 1 equipo']" variant="underlined">
                  ...
                  <v-col cols="12" md="6">
                    <v-file-input prepend-icon="mdi-image" variant="underlined" accept="image/*"
                      label="Imagen del jugador" v-model="form.image" @click:clear="form.imageUrl = ''"
                      @change="getImageSrc"></v-file-input>
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-select label="Tipo" :items="jugadorTypes" v-model="form.tipo" variant="underlined"
                      @update:modelValue="tipoChange"></v-select>
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-text-field label="Descripcion" hint="Curiosidades o peculiaridades del jugador"
                      :persistent-hint="true" variant="underlined" v-model="form.descripcion"></v-text-field>
                  </v-col>
                  <v-col cols="12" md="6" :class="{ 'd-none': isNotJugador }">
                    <v-select label="Posicion" :items="jugadorPositions" v-model="form.posicion"
                      variant="underlined"></v-select>
                  </v-col>
                  <v-col cols="12" md="6" :class="{ 'd-none': isNotJugador }">
                    <v-select label="Mano" :items="jugadorManos" v-model="form.mano" variant="underlined"></v-select>
                  </v-col>
                  <v-col cols="12" md="6" :class="{ 'd-none': isNotJugador }">
                    <v-text-field label="Dorsal" type="number" variant="underlined"
                      v-model="form.dorsal"></v-text-field>
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-text-field label="Especialidad" variant="underlined" v-model="form.especialidad"></v-text-field>
                  </v-col>
                  <v-col cols="12" md="6">
        ...
        <template v-slot:actions>
          <v-btn v-if="actionType == 'editar'" class="ms-5 mb-2" @click="
            databaseStore.eliminarJugador(jugador.id);
          closeDialog();
          " :loading="databaseStore.loadingDeleteDoc" prepend-icon="mdi-trash-can-outline" color="red"
            variant="outlined">
            Eliminar
          </v-btn>
          <v-spacer></v-spacer>
          <v-btn class="me-5 mb-2" type="submit" variant="outlined" color="green-darken-1" :loading="loadingData">
            Guardar
          </v-btn>
        </template>
      </v-card>
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      ...
      form.value.equipo = newVal.equipo;
    } else {
