<template>
  <v-expansion-panels>
    <v-expansion-panel v-for="equipo in databaseStore.equipos" :key="equipo.id" class="pa-0">
      ...
      <v-expansion-panel-text>
        <div class="d-flex justify-space-between">
          <v-btn @click="databaseStore.eliminarEquipo(equipo.id)" :loading="databaseStore.loadingDeleteDoc"
            prepend-icon="mdi-trash-can-outline" color="red" variant="outlined">
            Eliminar
          </v-btn>
          <v-btn @click="openDialog(equipo)" prepend-icon="mdi-pencil-outline" color="primary" variant="outlined">
            Editar
          </v-btn>
        </div>
      </v-expansion-panel-text>
    </v-expansion-panel>
    <EquipoFormModal :isOpen="isDialogOpen" actionType="editar" :equipo="equipo" @closeDialog="isDialogOpen = false" />
  </v-expansion-panels>
</template>
